...
# Re-exported so existing `from analysis.summaries import table_exists` callers keep working.
...
def build_summaries() -> dict[str, int]:
    ...
    with engine.connect() as conn:
        state_df = pd.read_sql_query(text(_STATE_AGG_SQL), conn)
        crop_df = pd.read_sql_query(text(_CROP_AGG_SQL), conn)
